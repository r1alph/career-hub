...
function NavBar() {
  ...
  return (
    <nav style={{ position:'fixed',bottom:0,left:0,right:0,zIndex:1000,background:'linear-gradient(180deg,transparent 0%,#06080F 15%)',padding:'12px 0 8px' }}>
      <div style={{ maxWidth:560,margin:'0 auto',display:'flex',justifyContent:'center',gap:2,background:'#0C1020',border:'1px solid #19223A',borderRadius:16,padding:'6px 4px',boxShadow:'0 -4px 30px rgba(0,0,0,0.5)' }}>
        {NAV_ITEMS.map(item => {
          const active = current === item.path || (item.path !== '/' && current.startsWith(item.path));
          return (
            <Link key={item.path} to={item.path} style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:2,padding:'5px 8px',borderRadius:10,textDecoration:'none',background:active?'#2563EB18':'transparent',border:active?'1px solid #2563EB30':'1px solid transparent',transition:'all 0.2s' }}>
              <span style={{ fontSize:14 }}>{item.icon}</span>
              <span style={{ fontSize:7,fontWeight:600,color:active?'#60A5FA':'#334155',letterSpacing:0.3 }}>{item.label}</span>
            </Link>
          );
        })}
      </div>
...
function HomePage() {
  const dashboards = [
    ...
    { path:'/5g-path', icon:'📡', title:'5G Engineer Pathway', desc:'9-section learning path with checkable concepts, curated resources, certifications', color:'#10B981', stat:'36 Weeks + 4-Month Plan' },
    { path:'/projects', icon:'🔧', title:'Project Portfolio', desc:'16 hands-on projects across 5 tiers mapped to jobs, companies, and skills.', color:'#8B5CF6', stat:'16 Projects + Steps' },
    ...
    { path:'/master-plan', icon:'📋', title:'30-Week Master Plan', desc:'Unified daily timetable combining all dashboards — skills, 5G, projects, certs, LinkedIn.', color:'#E8553D', stat:'30 Weeks · All-in-One' },
    { path:'/cgng', icon:'🎓', title:'CGNG Telecom Academy', desc:'4 courses with 28 industry-grade projects: 5G Mastery, O-RAN, AI/ML in Telecom, DevOps.', color:'#7C3AED', stat:'28 Industry Projects' },
  ];
    ...
    <div style={{ fontFamily:"'DM Sans',system-ui,sans-serif",background:'#06080F',color:'#CBD5E1',minHeight:'100vh',paddingBottom:80 }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .fd1{animation-delay:.1s;opacity:0}.fd2{animation-delay:.2s;opacity:0}.fd3{animation-delay:.3s;opacity:0}.fd4{animation-delay:.4s;opacity:0}.fd5{animation-delay:.5s;opacity:0}
      ...
      <div style={{ padding:'60px 20px 40px',textAlign:'center',background:'linear-gradient(180deg,#0D1225 0%,#06080F 100%)' }}>
        ...
        <div className="fade fd3" style={{ display:'flex',gap:8,justifyContent:'center',marginTop:16,flexWrap:'wrap' }}>
          {['THz Systems','5G/6G','Physical Layer Security','ML/DL','O-RAN','SDR/USRP','TelcoCloud'].map((s,i) => (
      ...
      <div style={{ maxWidth:600,margin:'0 auto',padding:'0 16px' }}>
        ...
        {dashboards.map((d,i) => (
          <Link key={d.path} to={d.path}>
            <div className={`fade fd${Math.min(i+1,5)}`} style={{ background:'linear-gradient(150deg,#0C1020,#111828)',border:'1px solid #19223A',borderRadius:14,padding:20,marginBottom:10,borderLeft:`4px solid ${d.color}`,transition:'all 0.25s',cursor:'pointer' }}>
    ...
    <HashRouter>
      <div style={{ paddingBottom:60 }}>
        <Routes>
          ...
          <Route path="/cgng" element={<CGNGAcademy />} />
        </Routes>
      </div>
      <NavBar />
    </HashRouter>
